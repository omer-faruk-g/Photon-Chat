...
class KnkApi {
  ...
  static Future<void> registerPresence(String myServerUrl, String fipId, String code, String name) async {
    try { await http.post(_u(myServerUrl, '/presence'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({'fipId': fipId, 'code': code, 'name': name})); } catch (_) {}
  }
  ...
  static Future<void> sendFriendRequest({required String toServerUrl, required String toFipId, required String fromFipId, required String fromCode, required String fromName, required String fromServerUrl, String? fromPublicKey}) async {
    await http.post(_u(toServerUrl, '/requests/$toFipId'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({'fromFipId': fromFipId, 'fromCode': fromCode, 'fromName': fromName, 'fromServerUrl': fromServerUrl, if (fromPublicKey != null) 'fromPublicKey': fromPublicKey}));
  }
  ...
  static Future<void> acceptFriendRequest({required String myServerUrl, required String myFipId, required String otherFipId}) async {
    try { await http.post(_u(myServerUrl, '/accept'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({'myFipId': myFipId, 'otherFipId': otherFipId})); } catch (_) {}
  }
  ...
  static Future<bool> isActive(String serverUrl, String fipId) async {
    try {
      final r = await http.post(_u(serverUrl, '/active'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({'fipIds': [fipId]}));
  ...
  static Future<bool> sendMessage({required String receiverServerUrl, required String chatKey, required String from, required String text, required int ts}) async {
    try {
      final r = await http.post(_u(receiverServerUrl, '/chat/$chatKey'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({'from': from, 'text': text, 'ts': ts}));
  ...
  static Future<void> deactivate(String myServerUrl, String fipId) async {
    try { await http.post(_u(myServerUrl, '/deactivate'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({'fipId': fipId})); } catch (_) {}
  }

  static Future<void> sendTyping(String myServerUrl, String chatKey, String fipId) async {
    try { await http.post(_u(myServerUrl, '/typing/$chatKey'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({'fipId': fipId, 'ts': DateTime.now().millisecondsSinceEpoch})); } catch (_) {}
  }
  ...
  static Future<Map<String, dynamic>?> createGroup(String myServerUrl, {required String ownerFipId, required String ownerName, required String name, required String ownerServerUrl}) async {
    try {
      final r = await http.post(_u(myServerUrl, '/groups'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({'ownerFipId': ownerFipId, 'ownerName': ownerName, 'name': name, 'ownerServerUrl': ownerServerUrl}));
  ...
  static Future<void> sendGroupJoinRequest(String ownerServerUrl, String groupId, {required String fromFipId, required String fromName, required String fromServerUrl}) async {
    try { await http.post(_u(ownerServerUrl, '/groups/$groupId/join-requests'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({'fromFipId': fromFipId, 'fromName': fromName, 'fromServerUrl': fromServerUrl})); } catch (_) {}
  }
  ...
  static Future<void> acceptGroupMember(String myServerUrl, String groupId, {required String fipId, required String name, required String serverUrl}) async {
    try { await http.post(_u(myServerUrl, '/groups/$groupId/members'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({'fipId': fipId, 'name': name, 'serverUrl': serverUrl})); } catch (_) {}
  }
  ...
  static Future<void> sendGroupMessage(List<String> memberServerUrls, String groupId, {required String from, required String fromName, required String text, required int ts}) async {
    for (final url in memberServerUrls) {
      try { await http.post(_u(url, '/groups/$groupId/messages'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({'from': from, 'fromName': fromName, 'text': text, 'ts': ts})); } catch (_) {}
    }
  }
  ...
  static Future<void> muteGroupMember(String ownerServerUrl, String groupId, String fipId) async {
    try { await http.post(_u(ownerServerUrl, '/groups/$groupId/muted'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({'fipId': fipId})); } catch (_) {}
  }
  ...
  static Future<void> sendGroupKey(String ownerServerUrl, String groupId, String memberFipId, String encryptedKey) async {
    try { await http.post(_u(ownerServerUrl, '/groups/$groupId/key/$memberFipId'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({'encryptedKey': encryptedKey})); } catch (_) {}
  }
  ...
  static Future<String?> getGroupKey(String ownerServerUrl, String groupId, String myFipId) async {
    try {
      ...
      if (r.statusCode == 200) return (jsonDecode(r.body) as Map<String, dynamic>)['encryptedKey'] as String?;
    } catch (_) {}
    return null;
  }
  ...
  static Future<String> chatWithPulseAI(String myServerUrl, List<Map<String, String>> messages) async {
    try {
      final r = await http.post(_u(myServerUrl, '/ai/chat'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({'messages': messages})).timeout(const Duration(seconds: 30));
      if (r.statusCode == 200) return (jsonDecode(r.body) as Map<String, dynamic>)['reply'] as String? ?? 'Yanıt alınamadı.';
      return (jsonDecode(r.body) as Map<String, dynamic>)['error'] as String? ?? 'Bir hata oluştu.';
    } catch (_) {
      return 'Pulse AI\'e ulaşılamadı.';
    }
  }
}
